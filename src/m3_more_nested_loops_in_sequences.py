...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 998
    answer = largest_number(([98, 93], [-222234, 998], [231]))
    print('Expected and actual are:', expected, answer)
...
def run_test_largest_negative_number():
    ...
    # Test 4:
    expected = -3
    answer = largest_negative_number(([98, -93, -3], [-222234, 998, 0], [231]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        sub_list = seq_seq[k]
        for a in range(len(sub_list)):
            if (largest is None)and(sub_list[a] < 0):
                largest = sub_list[a]
            if (sub_list[a] < 0)and(largest < sub_list[a]):
                largest = sub_list[a]
    return largest
